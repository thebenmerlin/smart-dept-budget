...
async function resetDatabase() {
  ...
  try {
    ...
    // Reset sequences
    await sql`ALTER SEQUENCE IF EXISTS departments_id_seq RESTART WITH 1`;
    await sql`ALTER SEQUENCE IF EXISTS categories_id_seq RESTART WITH 1`;
    await sql`ALTER SEQUENCE IF EXISTS users_id_seq RESTART WITH 1`;
    await sql`ALTER SEQUENCE IF EXISTS expenses_id_seq RESTART WITH 1`;
...
async function seedUsers() {
  ...
  const users = [
    ...
    { id: 2, name: 'Dr. Rajesh Kumar', email: 'hod@rscoe.edu.in', role: 'hod' },
    { id: 3, name: 'Prof. Sneha Patil', email: 'staff@rscoe. edu.in', role: 'staff' },
  ];
  ...
  for (const user of users) {
    try {
      await sql`
        ...
        VALUES (${user.id}, 1, ${user. name}, ${user. email}, ${passwordHash}, ${user.role}, true)
...
async function seedSampleData() {
  ...
  console. log('  Created budget allotments');
  ...
  const expenses = [
    ...
    { category_id: 7, amount: 25000, vendor:  'AWS', date: '2024-10-15', description:  'AWS credits for student projects', status: 'approved' },
    { category_id: 8, amount: 8000, vendor: 'Staples Office', date: '2024-11-01', description: 'Office stationery and supplies', status: 'pending' },
  ];
  ...
  for (const expense of expenses) {
    try {
      ...
      `;
      console.log('  Created expense: ' + expense.vendor + ' (' + expense.status + ')');
    } catch (error:  any) {
